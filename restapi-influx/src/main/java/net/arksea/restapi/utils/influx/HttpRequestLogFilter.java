...
public class HttpRequestLogFilter implements Filter {
    ...
    public HttpRequestLogFilter(IHttpRequestLogFilterConfig config, IRequestLogger requestLogger, boolean delayCountRequest) {
        this.config = config;
    ...
    private void getGroupNameAndDoFilter(final HttpServletRequest req,HttpServletResponse resp,final FilterChain chain) throws IOException {
        ...
        try {
            name = config.getName(req, handledWrapper);
            group = config.getGroup(req, handledWrapper);
        } catch (Exception ex) {
